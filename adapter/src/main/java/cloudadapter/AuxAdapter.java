...
public class AuxAdapter {
    ...
    /*
     * Commented out: the port should be one not busy, so fixing the port is not a good thing.
     */
    ...
    public static String getRunningStatusCF(String email, String password, String appName) throws Cloud4SoaException {
        ...
        try {
            ...
            if (status.equals(AppState.STARTED)) {
                ...
                try {
                    String urlstr = getApplicationUriCF(email, password, appName);
    ...
    */
    ...
    /**
     * Class to encapsulate the needed info to connect to cloudfoundry tunnel.
     * 
     * It wouldn't be necessary if Java could provide dicts on the fly like pyhton.
     */
    static class CFTunnelInfo {
        private DatabaseCredentials credentials;
        private int localPort;
        
        CFTunnelInfo(DatabaseCredentials credentials, int localPort) {
    ...
    public static CFTunnelInfo openTunnelCF(String email, String password, String dbtype, String dbname) throws Cloud4SoaException {
        CloudFoundryClient client = null;
        ...
        try {
            ...
            /*
             * 0 to use first free port
             */
            InetSocketAddress local = new InetSocketAddress(AuxAdapter.TUNNEL_LOCAL_HOST, 0);
            ...
            server.start();
            System.out.println("Tunnel is running on " + AuxAdapter.TUNNEL_LOCAL_HOST + " port " + server.getLocalPort() + " with auth=" + auth);
            if (svc_vhost != null) {
            ...
            DatabaseCredentials cred = new DatabaseCredentials(svc_username, svc_passwd, svc_dbname);
            int localPort = server.getLocalPort();
            return new CFTunnelInfo(cred, localPort);
