...
//////////////DB RELATED METHODS//////////////////
    ...
    public static void downloadDBCF(String publicKey, String secretKey, String dbtype, String dbname, String fileToStore) throws Cloud4SoaException {
        ...
        try {
            AuxAdapter.CFTunnelInfo info = AuxAdapter.openTunnelCF(publicKey, secretKey, dbtype, dbname);
            DatabaseCredentials cred = info.getCredentials();
            String port = String.valueOf(info.getLocalPort());
            //DatabaseCredentials cred = new DatabaseCredentials("aa","aa");
            utils.DatabaseHelper dbhelp = new DatabaseHelper();
            ///forMySQL
            dbhelp.storeDB("127.0.0.1", port, cred.user, cred.pass, cred.tunnel_name, fileToStore);
    ...
    public static void restoreDBCF(String publicKey, String secretKey, String dbtype, String dbname, String fileToStore) throws Cloud4SoaException {
        ...
        try {
            ...
            utils.DatabaseHelper dbhelp = new DatabaseHelper();
            ///forMySQL
            dbhelp.restoredb("127.0.0.1", port, cred.user, cred.pass, cred.tunnel_name, fileToStore);
        } catch (Exception ex) {
            Logger.getLogger(Adapter.class.getName()).log(Level.SEVERE, null, ex);
            throw new Cloud4SoaException("Erron when downloading db with name " + dbname + " at CloudFoundry. Message:" + ex.getMessage());
        } finally {
    ...
    public static String downloadDB_REST(String host, String port,
            String dbtype, String dbname, String dbuser, String dbpassword,
            String localfile) throws Cloud4SoaException {
        String ret = null;
        ...
        try {
            ...
            Logger.getLogger(Adapter.class.getName()).log(Level.SEVERE, null, ex);
            throw new Cloud4SoaException("Erron when downloading db with name "
                    + dbname + " at host " + host + ", for user " + dbuser
                    + ". Message:" + ex.getMessage());
        }
        return ret;
    }

    public static String restoreDB_REST(String host, String port, String dbtype, String dbname, String dbuser, 
            String dbpassword, String localfile) throws Cloud4SoaException {
        ...
        try {
            ...
            throw new Cloud4SoaException(
                    "Erron when restoring db from file, with db name " + dbname
                            + " at host " + host + ", for user " + dbuser
                            + ". Message:" + ex.getMessage());
        }
