...
import cloudfoundry.VcapServicesReader;
...
public class Adapter {
    ...
    private static String api_key; //email from CloudFoundry account
    private static String api_secret; //password from CloudFoundry account  
...
//            getDatabaseDetailsImplOld(dbid, response);
            /*
             * read VCAP_SERVICES and return data
             */
            String json = System.getenv(VCAP_SERVICES);
            if (json == null) {
                return null;
            }
            JsonNode node = VcapServicesReader.parse(json);
            Database database = VcapServicesReader.fillDatabase(node, dbid);
    ...
    /*
     * Implementation that don't return needed db params (dbname, user, pwd...) 
     */
    private void getDatabaseDetailsImplOld(String dbid,
            DatabaseResponse response) throws Exception {
...
//        logger.log(Level.INFO, );
        ...
        try {
            ...
            /*
             * This is bit tricky. The side effect of the next instruction
             * restarts the own adapter.
             */
    	...
    	Logger.getLogger(Adapter.class.getName()).log(Level.FINE, "Exiting createDatabase()");
    ...
    public ExtendedMonitorResponse doExtendedMonitoring(@PathParam("applicationUrl") String applicationUrl) throws ClientWebApplicationException {
        ...
        System.out.println("applicationUrlapplicationUrl:"+applicationUrl);
        ...
        if ((applicationUrl != null && !"".equals(applicationUrl)) && vm.checkAppMetricsAvailability(applicationUrl) == 200) {
            System.out.println("metrics retrieved from application1");
            ...
            System.out.println("metrics retrieved from application2");
        } else {
